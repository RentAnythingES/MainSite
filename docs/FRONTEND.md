...
Product-family and kit-related product cards serve their existing reviewed source files directly.
...
Category routes use `CategoryProductCatalogue`. It renders the full active category
membership as one continuous, dense `ProductCard` grid in both locales. The compact
catalogue bar contains only the heading and live count. Do not split the grid into
vertical product-type sections: small groups waste rows and push products down the
page. Optional comparison/family links render after the full catalogue. Do not
...
| `/admin/asset-accounting` | Client | Product purchase register with ownership, straight-line depreciation, and rental revenue from purchase date |
...
commercial category pathway, and schema role from the matching English guide.
Evergreen festival guides use `Article` in both locales; place and attraction
guides use `TouristDestination`. Run `npm run audit:discover-locales` after adding
or translating a guide.
