...
export function buildBookingDocumentPdf(document: BookingDocument, booking: BookingForPdf): Uint8Array {
  ...
  content += pdfText("Valencia, Spain", 330, 606, 9, "F1");
  ...
  let y = 340;
  if (isRefund) {
    content += pdfText("Refund processed", 58, y, 9, "F1");
    ...
    content += pdfText(formatMoney(document.subtotal_cents, document.currency), 484, y, 9, "F1");
    y -= 22;
    content += pdfText("Delivery", 58, y, 9, "F1");
    content += pdfText(formatMoney(document.delivery_fee_cents, document.currency), 484, y, 9, "F1");
    y -= 22;
    if (document.collection_fee_cents > 0) {
      content += pdfText("Collection", 58, y, 9, "F1");
      content += pdfText(formatMoney(document.collection_fee_cents, document.currency), 484, y, 9, "F1");
      y -= 22;
    }
    content += pdfText("Tax", 58, y, 9, "F1");
    content += pdfText(formatMoney(document.tax_cents, document.currency), 484, y, 9, "F1");
  }

  content += pdfLine(390, 248, 564, 248, 0.5);
  content += pdfText(isRefund ? "Refund total" : "Total paid", 390, 226, 12, "F2");
  ...
  content += pdfText("Generated by RentAnything.es admin from booking and payment records.", 48, 52, 8, "F1");
