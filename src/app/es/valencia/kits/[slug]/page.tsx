  ...
  return (
    <>
      ...
      <nav className="bg-neutral-50 border-b border-border py-3">
        <div className="container-site">
          <ol className="flex items-center gap-2 text-sm text-neutral-500">
            <li><Link href="/es" className="hover:text-brand transition-colors">Inicio</Link></li><li>/</li>
            <li><Link href="/es/valencia" className="hover:text-brand transition-colors">Valencia</Link></li><li>/</li>
            <li><Link href="/es/valencia/kits" className="hover:text-brand transition-colors">Kits</Link></li><li>/</li>
      ...
      <section className="section bg-white">
        <div className="container-site">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <span className={`inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${accentClasses[bundle.accent]}`}>{bundle.eyebrow}</span>
              <h1 className="mt-5 text-4xl md:text-5xl font-extrabold tracking-tight">{bundle.name}</h1>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {bundle.includedItems.map((item) => (
                  <div key={item.name} className="rounded-2xl border border-border bg-neutral-50 p-4">
                    <div className="flex items-start gap-3"><span className="mt-1 h-5 w-5 rounded-full bg-brand/10 text-brand flex items-center justify-center text-xs font-bold">✓</span><div><p className="font-semibold text-neutral-800">{item.quantity ? `${item.quantity} ${item.name}` : item.name}</p>{item.note && <p className="mt-1 text-sm text-neutral-500">{item.note}</p>}</div></div>
                  </div>
                ))}
              </div>
            </div>
            <aside className="card p-6 bg-white"><h2 className="text-xl font-bold mb-4">Recomendado para</h2><ul className="space-y-3">{bundle.bestFor.map((item) => <li key={item} className="flex gap-3 text-sm text-neutral-600"><span className="text-brand">•</span>{item}</li>)}</ul></aside>
          </div>
        </div>
      </section>
      ...
      {relatedGuides.length > 0 && (
        <section className="section bg-white"><div className="container-site"><h2 className="text-3xl font-bold mb-6">Guías relacionadas de Valencia</h2><div className="grid md:grid-cols-3 gap-6">{relatedGuides.map((guide) => <Link key={guide.slug} href={`/es/blog/${guide.slug}`} className="card p-6 bg-white hover:shadow-md transition-shadow group"><span className="badge badge-brand capitalize mb-3">{guide.category}</span><h3 className="font-bold text-lg group-hover:text-brand transition-colors">{guide.title}</h3><p className="mt-2 text-sm text-neutral-500 leading-relaxed">{guide.excerpt}</p><span className="mt-4 inline-block text-sm font-bold text-brand">Leer guía →</span></Link>)}</div></div></section>
      )}

      <section className="section bg-neutral-50"><div className="container-site"><h2 className="text-3xl font-bold mb-6">Preguntas sobre este kit</h2><div className="grid md:grid-cols-2 gap-4">{bundle.faqs.map((faq) => <div key={faq.question} className="card p-6 bg-white"><h3 className="font-bold text-neutral-800">{faq.question}</h3><p className="mt-2 text-sm text-neutral-600 leading-relaxed">{faq.answer}</p></div>)}</div></div></section>

      <section className="section bg-white">
        <div className="container-site">
          <div className="flex items-end justify-between gap-6 mb-6"><div><span className="badge badge-brand mb-3">Más kits</span><h2 className="text-3xl font-bold">Otras formas de empezar</h2></div><Link href="/es/valencia/kits" className="hidden sm:inline-block text-sm font-bold text-brand">Ver todos los kits →</Link></div>
          <div className="grid md:grid-cols-3 gap-6">{otherBundles.map((item) => <BundleCard key={item.slug} bundle={item} compact basePath="/es/valencia/kits" ctaLabel="Ver kit →" />)}</div>
        </div>
      </section>
    </>
  );
