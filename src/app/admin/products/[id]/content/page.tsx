...
export default function ProductContentPage() {
  ...
  return <div className="max-w-5xl">
    ...
    <div className="mb-6 rounded-xl border border-neutral-800 bg-neutral-900 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-semibold text-white">Publication readiness</p><p className="mt-1 text-sm text-neutral-400">{readiness.ready ? "This product has the required content and operational foundations." : "Keep this product inactive until the missing items are completed."}</p></div><select value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-lg border border-neutral-700 bg-neutral-800 px-3 py-2 text-sm text-white"><option value="draft">Draft</option><option value="facts_verified">Facts verified</option><option value="content_ready">Content ready</option></select></div>
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-neutral-800 bg-neutral-900 p-6"><h2 className="mb-4 font-semibold text-white">Primary image record</h2>{product.image_url ? <div className="grid gap-4 md:grid-cols-[180px_1fr]"><img src={product.image_url} alt="Current product" className="h-36 w-full rounded-lg bg-neutral-950 object-contain" /><div className="space-y-4"><div><label className={labelClass}>Alt text</label><input value={primaryImage.alt_text} onChange={(event) => setPrimaryImage({ ...primaryImage, alt_text: event.target.value })} className={inputClass} /></div><div className="grid gap-4 md:grid-cols-2"><div><label className={labelClass}>Image-use status</label><select value={primaryImage.rights_status} onChange={(event) => setPrimaryImage({ ...primaryImage, rights_status: event.target.value })} className={inputClass}><option value="unknown">Needs confirmation</option><option value="owned">Owned by RentAnything</option><option value="licensed">Licensed</option><option value="manufacturer_approved">Manufacturer approved</option></select></div><div><label className={labelClass}>Internal source URL</label><input value={primaryImage.source_url} onChange={(event) => setPrimaryImage({ ...primaryImage, source_url: event.target.value })} className={inputClass} /></div></div></div></div> : <p className="text-sm text-amber-300">Upload an approved product image from the product editor first.</p>}</section>
      ...
      <div className="flex justify-end"><button onClick={() => void save()} disabled={saving} className="rounded-lg bg-teal-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-teal-500 disabled:opacity-50">{saving ? "Saving..." : "Save content"}</button></div>
    </div>
  </div>;
}
