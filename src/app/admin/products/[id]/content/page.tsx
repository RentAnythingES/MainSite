...
export default function ProductContentPage() {
  ...
  return <div className="max-w-5xl">
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-neutral-800 bg-neutral-900 p-6"><h2 className="font-semibold text-white">Primary image review</h2><p className="mb-4 mt-1 text-sm text-neutral-400">Record the image source and the basis for publication. Owner approval records an editorial decision; it does not claim a supplier licence.</p>{product.image_url ? <div className="grid gap-4 md:grid-cols-[180px_1fr]"><img src={product.image_url} alt="Current product" className="h-36 w-full rounded-lg bg-neutral-950 object-contain" /><div className="space-y-4"><div><label className={labelClass}>Accessible image description</label><input value={primaryImage.alt_text} onChange={(event) => setPrimaryImage({ ...primaryImage, alt_text: event.target.value })} className={inputClass} /></div><div className="grid gap-4 md:grid-cols-2"><div><label className={labelClass}>Publication approval</label><select value={primaryImage.rights_status} onChange={(event) => setPrimaryImage({ ...primaryImage, rights_status: event.target.value })} className={inputClass}><option value="unknown">Not confirmed — blocks publication</option><option value="owned">We took or created this image</option><option value="licensed">We have permission or a licence</option><option value="manufacturer_approved">Manufacturer approved it for our catalogue</option><option value="owner_approved">Business owner approved publication</option></select><p className="mt-1 text-xs text-neutral-500">Do not guess. The selected reason is kept as an internal review record.</p></div><div><label className={labelClass}>Where did the image come from?</label><input value={primaryImage.source_url} onChange={(event) => setPrimaryImage({ ...primaryImage, source_url: event.target.value })} placeholder="Source page, supplier file, or internal note" className={inputClass} /></div></div></div></div> : <p className="text-sm text-amber-300">Upload an approved product image from the product editor first.</p>}</section>
